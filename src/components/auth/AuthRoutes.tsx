...
export function ResetPasswordPage() {
  ...
  return <Shell><h1 className="font-black text-3xl">SET A NEW PASSWORD</h1><p className="font-bold opacity-60 mt-2">Choose a new password for your datings.lol account.</p><form onSubmit={submit} className="space-y-3 mt-5"><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="New password" autoComplete="new-password" className="w-full h-12 border-[2px] border-black rounded-xl px-3 font-bold text-[#111] placeholder:text-gray-500" /><input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="Confirm password" autoComplete="new-password" className="w-full h-12 border-[2px] border-black rounded-xl px-3 font-bold text-[#111] placeholder:text-gray-500" /><button className="w-full h-12 bg-[#FFE066] border-[2px] border-black rounded-xl font-black uppercase brutal-shadow-sm">Update password</button></form>{(message || authError) && <p className="mt-4 bg-[#FDA4AF] border-[2px] border-black rounded-xl p-3 font-bold text-sm">{message || authError}</p>}</Shell>;
}
